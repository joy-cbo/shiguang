...
`plugins/registry.ts` 通过 `import.meta.glob` 自动扫描 `plugins/*/plugin.json`。**新插件只需建文件夹 + 写 plugin.json，不需要改 registry.ts 或任何共享文件。** 拉上游更新零合并冲突。
...
# 4. 在 server/api/ 和 pages/ 创建重导出文件（一行 export）
```
